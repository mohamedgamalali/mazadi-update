...
exports.getTest = async (req,res,next)=>{
    ...
    try{
        ...
        const totalProducts    = await Products.find({approve:'approved',bidStatus:{$ne:'ended'}}).countDocuments();
             
        const totalAskProducts = await AskProduct.find({approve:'approved',ended:false}).countDocuments();
            ...
            if(!Array.isArray(id)){
                ...
                id.forEach(async i=>{
                   ...
                   product = await Products.findById(mongoose.Types.ObjectId(i));
                    ...
                    const Delete    = await Products.findByIdAndDelete(mongoose.Types.ObjectId(i));
                    const catigory  = await Catigory.findOne({ _id: product.catigory });
                    const user      = await User.findOne({ _id: product.user});
