...
exports.putProducts = async(req,res,next)=>{
    ...
    if(image.length===0){
        ...
        await user.save() ;
        res.status(200).json({state:1,message:'product created Sucessfullt',productId:savedProduct._id});
    }catch(err){
        if(!err.statusCode){
            err.statusCode = 500; 
        }
...
exports.getProducts = async(req,res,next)=>{
    ...
    try{
        ...
        if(filter=='1'){
             totalProducts = await Product.find({catigory:catigory,approve:'approved'})
             .where('bidStatus')
             .ne('ended')
             .countDocuments();
            ...
            .ne('ended')
            .countDocuments();
             products   = await Product.find({catigory:catigory,approve:'approved'}).sort({createdAt: -1}) 
            .where('bidStatus')
            .ne('ended')
            .select('approve')
            .select('imageUrl')
            ...
            .ne('ended')
            .countDocuments();
            products   = await Product.find({catigory:catigory,approve:'approved'}).sort({TotalPid: -1}) 
            .where('bidStatus')
            .ne('ended')
            .select('approve')
            .select('imageUrl')
        ...
        .limit(productPerPage);
            ...
            totalProducts = await Product.find({catigory:catigory,approve:'approved',city:filter})
            .where('bidStatus')
            .ne('ended')
            .countDocuments();
             products   = await Product.find({catigory:catigory,approve:'approved',city:filter})
             .where('bidStatus')
            .ne('ended')
            .select('approve')
            .select('imageUrl')
...
exports.getCatigory = async(req,res,next)=>{
    ...
    try{
        ...
        return res.status(200).json({state:1,catigories:catigories,totalUsers:users})
...
exports.putPid = async(req,res,next)=>{
    ...
    let find = false ;
    try{
       const product = await Product.findByIdAndUpdate(prodId).populate('user').populate('lastPid');
       if(!product){
        const error = new Error('product not found!!..');
        error.statusCode = 404 ;
        throw error ;
       ...
       if(lastUser){
        ...
        const n = await sendNotfication.send(lastUser.FCMJwt,body,notfi,[lastUser._id]);    
       }
    ...
    const n2 = await sendNotfication.send(finalProduct.user.FCMJwt,body2,notfi2,[finalProduct.user._id]);
...
exports.getAskProduct = async(req,res,next)=>{
    ...
    try{
        if(filter=='1'){
            total = await AskProduct.find({catigory:categoryID,approve:"approved",ended:false}).countDocuments();
            askProduct = await AskProduct.find({catigory:categoryID,approve:"approved",ended:false})
            .select('desc')
            .select('approve')
            .skip((page-1)*productPerPage)
            .limit(productPerPage);
            ...
            total = await AskProduct.find({catigory:categoryID,approve:"approved",city:filter,ended:false}).countDocuments();
            askProduct = await AskProduct.find({catigory:categoryID,approve:"approved",city:filter,ended:false})
            .select('desc')
            .select('approve')
            .skip((page-1)*productPerPage)
            .limit(productPerPage);
...
exports.putAskProductBid = async(req,res,next)=>{
    ...
    if(image.length===0){
        ...
        const n = await sendNotfication.send(finalProd.user.FCMJwt,body,notfi,[finalProd.user._id]);
        io.getIO().emit('askProducts',{action:'creat',
