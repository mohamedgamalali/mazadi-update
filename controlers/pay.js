...
exports.postPay = async (req,res,next)=>{
    ...
    try{
        ...
        const product  = await Product.findOne({_id:productId}).populate('lastPid').populate('user');
        ...
        const n = await sendNotfication.send(product.user.FCMJwt,Sbody,Snotfi,[product.user._id]);
...
exports.postOrderNotf = async(req,res,next)=>{
    ...
    if(bid.length>0){
        ...
        const n = await sendNotfication.send(user.FCMJwt,Sbody,Snotfi,[user._id]);
...
exports.postPayManProd = async (req,res,next)=>{
    ...
    try{
     ...
     const product = await Product.findById(productId).populate('user').populate('lastPid');
     if(!product){
        const error = new Error('product not found');
        error.statusCode = 404 ;
        throw error ;
...
exports.postPayManOrder = async (req,res,next)=>{
    ...
    if(bid.length==0){
        const error = new Error('bid not found or may not be selected from the owner');
        error.statusCode = 404 ;
        throw error ;
    }
